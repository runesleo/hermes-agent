...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Disable with platforms.telegram.extra.publish_commands=false (or
            # HERMES_TELEGRAM_PUBLISH_COMMANDS=false) for private bots that should
            # not expose their command surface in other Telegram chats.
            try:
                publish_commands = self._coerce_bool_extra("publish_commands", True)
                env_publish_commands = os.getenv("HERMES_TELEGRAM_PUBLISH_COMMANDS") or os.getenv("TELEGRAM_PUBLISH_COMMANDS")
                if env_publish_commands is not None:
                    publish_commands = env_publish_commands.strip().lower() in ("1", "true", "yes", "on")

                if not publish_commands:
                    await self._bot.delete_my_commands()
                    logger.info("[%s] Telegram command menu disabled by config", self.name)
                else:
                ...
                logger.warning(
                    "[%s] Could not configure Telegram command menu: %s",
                    self.name,
                    e,
                    exc_info=True,
                )
