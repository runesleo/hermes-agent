...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"smart_model_routing": routing_config},
    )
    ...
    shell._smart_model_routing = dict(routing_config)
...
def test_cli_turn_routing_reloads_smart_routing_from_disk(monkeypatch):
    cli = _import_cli()
    ...
    shell._smart_model_routing = {
        "enabled": True,
        "cheap_model": {"provider": "copilot-acp", "model": "gpt-4.1"},
        "max_simple_chars": 160,
        "max_simple_words": 28,
    }
    ...
    monkeypatch.setattr(
        "hermes_cli.config.load_config",
        lambda: {"smart_model_routing": {"enabled": False}},
    )

    result = shell._resolve_turn_agent_config("按优先级推动")
